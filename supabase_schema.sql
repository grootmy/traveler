...
CREATE POLICY "방 멤버는 모두가 볼 수 있습니다" ON public.room_members
  FOR SELECT USING (true);
CREATE POLICY "인증된 사용자는 방에 참여할 수 있습니다" ON public.room_members
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "사용자는 자신의 방 멤버십만 업데이트할 수 있습니다" ON public.room_members
  FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "사용자는 자신의 방 멤버십만 삭제할 수 있습니다" ON public.room_members
...
CREATE TABLE IF NOT EXISTS public.route_votes (
  ...
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
  vote_value INTEGER DEFAULT 0, -- 0: 중립, 1: 좋아요, -1: 싫어요
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
...
CREATE POLICY "투표는 방 멤버만 볼 수 있습니다" ON public.route_votes
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE routes.id = route_votes.route_id
      AND room_members.user_id = auth.uid()
    )
  );
...
CREATE TABLE IF NOT EXISTS public.selected_routes (
  ...
  room_id UUID REFERENCES public.rooms(id) ON DELETE CASCADE NOT NULL,
  route_id UUID REFERENCES public.routes(id) ON DELETE CASCADE NOT NULL,
  selected_by UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
...
CREATE POLICY "방 소유자만 최종 경로를 선택할 수 있습니다" ON public.selected_routes
  FOR INSERT WITH CHECK (
    EXISTS (
      ...
      WHERE rooms.id = selected_routes.room_id
